...
class TwitterScraping():
    ...
    def scrape_worker(self,query):
        for i, tweet in enumerate(sntwitter.TwitterSearchScraper(query).get_items()):
            ...
            self.tweets_list.append({
                "kullanıcı adı":tweet.user.username, 
                "tarih":tweet.date,
    ...
    def search_user(self):
        query = f"(from:{self.account}) until:{self.until} since:{self.since}"
        self.scrape_worker(query)

    def search_hashtag(self):
        query = f"(#{self.hashtag}) until:{self.until} since:{self.since}"
        self.scrape_worker(query)

    def search_keyword(self):
        query = f"({self.word}) until:{self.until} since:{self.since}"
        post_id = self.scrape_worker(query)
        return post_id
...
class Youtube ():
    ...
    def main(self):
        ...
        try:
            ...
            sys.exit(1)
        return real_dic
# print(Youtube('https://www.youtube.com/watch?v=ftFEqvsend0',10).main())